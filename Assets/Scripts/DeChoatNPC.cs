...
public class DeChoatNPC : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        if (player == null)
        {
            ...
            if (p != null) {
                 player = p.transform;
                 Debug.Log("DeChoatNPC: Player found by tag.");
            } else {
                 Debug.LogError("DeChoatNPC: Player NOT found! Make sure Player tag is set.");
            }
        }
        ...
        if (chatBubble == null) Debug.LogError("DeChoatNPC: ChatBubble is missing! Run the Setup Tool.");
    }
    ...
    void Update()
    {
        ...
        {
            Debug.Log("DeChoatNPC: 'F' pressed. Starting interaction...");
            if (!isTalking)
            {
                StartInteraction();
            }
    ...
    System.Collections.IEnumerator ShowDialogueRoutine()
    {
        if (chatBubble != null)
        {
            ...
            {
                Debug.Log($"DeChoatNPC SAYS: {sentence}");
                chatBubble.Setup(sentence);
                yield return new WaitForSeconds(timePerSentence);
            }
            chatBubble.Hide();
        } else {
            Debug.LogError("DeChoatNPC: ChatBubble is null in Coroutine!");
        }
        EndInteraction();
    }
    ...
    public void EndInteraction()
    {
        ...
        // Hoàn thành nhiệm vụ "Nói chuyện với Dế Choắt"
        if (QuestUIManager.Instance != null)
            QuestUIManager.Instance.CompleteQuest("talk_dechoat");
